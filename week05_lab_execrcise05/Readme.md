...
- Why is `express.Router()` used in Express.js applications, and how does it benefit the code structure?
    - express.Router() is used to create separate route modules in Express. 
      - It lets you organize routes into different files instead of keeping them all in index.js. 
      - This makes the code cleaner, modular, and easier to maintain, especially in large apps. 
      - Example: all /user routes can go in users.js, then you just import them into index.js.
...
- How would you implement error handling in the Express routes to ensure that any issues (such as file not found or server errors) are appropriately handled? Provide an example.
    - Error handling in Express is done with middleware that takes 4 parameters: (err, req, res, next). 
      - If something goes wrong (like file not found or invalid JSON), the error middleware will catch it and send a proper response.
      - Example:
      - app.use((err, req, res, next) => {
        console.error(err.stack);   // log error
        res.status(500).send("Server Error");  // send response
        });
...
- Explain how the `app.listen(process.env.port || 8081)` line works and why it's useful in production environments.
    - app.listen(process.env.port || 8081) means:
      - First, check if there is a port number set in the environment (e.g., by Heroku, AWS, or Docker). 
      - If not, use 8081 as the default port.
